...
func main() {
	...
	io := filepath.FromSlash("fixedbugs/bug345.dir/io.go")
	main := filepath.FromSlash("fixedbugs/bug345.dir/main.go")
	run("go", "tool", a+"g", io)
	errchk(main, "main.go", "go", "tool", a+"g", "-e", main)
	os.Remove("io." + a)
}
...
func errchk(full, short string, name string, args ...string) {
	cmd := exec.Command(name, args...)
	out, err := cmd.CombinedOutput()
	if err == nil {
		fmt.Println("expected compilation failure:", full)
		os.Exit(1)
	}
	err = errorCheck(string(out), full, short)
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
