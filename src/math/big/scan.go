...
func (s *scanner) chunkSize(size int) (int, *Int) {
...
func (s *scanner) power(k uint) *Int {
	for i := len(s.powers); i <= int(k); i++ {
		...
		if i == 0 {
			...
			z.Exp(NewInt(1e14), NewInt(quadraticScanThreshold/14), nil)
		} else {
...
func (s *scanner) scan(z *Int, str string) {
