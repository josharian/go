...
func overflow(v Val, t *Type) {
	...
	switch v.Ctype {
	case CTINT, CTRUNE:
		Yyerror("constant %v overflows %v", Bconv(v.U.(*Mpint), 0), Tconv(t, 0))

	case CTFLT:
		Yyerror("constant %v overflows %v", Fconv(v.U.(*Mpflt), obj.FmtSharp), Tconv(t, 0))
...
func Convconst(con *Node, t *Type, val *Val) {
	...
	if Isint[tt] {
		...
		case CTINT, CTRUNE:
			i = Mpgetfix(val.U.(*Mpint))

		case CTBOOL:
			i = int64(bool2int(val.U.(bool)))
