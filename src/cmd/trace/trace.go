...
func generateTrace(params *traceParams) (traceviewer.Data, error) {
	...
	ctx.emit(&traceviewer.Event{Name: "thread_name", Phase: "M", Pid: 0, Tid: trace.GCP, Arg: &NameArg{"GC"}})
	ctx.emit(&traceviewer.Event{Name: "thread_sort_index", Phase: "M", Pid: 0, Tid: trace.GCP, Arg: &SortIndexArg{-6}})

	ctx.emit(&traceviewer.Event{Name: "thread_name", Phase: "M", Pid: 0, Tid: trace.NetpollP, Arg: &NameArg{"Network"}})
	ctx.emit(&traceviewer.Event{Name: "thread_sort_index", Phase: "M", Pid: 0, Tid: trace.NetpollP, Arg: &SortIndexArg{-5}})
...
func (ctx *traceContext) emitHeapCounters(ev *trace.Event) {
	...
	ctx.emit(&traceviewer.Event{Name: "Heap", Phase: "C", Time: ctx.time(ev), Pid: 1, Arg: &heapCountersArg{ctx.heapAlloc, diff}})
}
...
func (ctx *traceContext) emitGoroutineCounters(ev *trace.Event) {
	...
	ctx.emit(&traceviewer.Event{Name: "Goroutines", Phase: "C", Time: ctx.time(ev), Pid: 1, Arg: &goroutineCountersArg{ctx.grunning, ctx.grunnable}})
}
...
func (ctx *traceContext) emitThreadCounters(ev *trace.Event) {
	...
	ctx.emit(&traceviewer.Event{Name: "Threads", Phase: "C", Time: ctx.time(ev), Pid: 1, Arg: &threadCountersArg{ctx.prunning, ctx.insyscall}})
}
