...
func rewriteValuegeneric_OpMove_20(v *Value) bool {
	...
	// cond: t1.(*types.Type).Compare(t2.(*types.Type)) == types.CMPeq && isSamePtr(tmp1, tmp2) && ((disjoint(src, s, tmp2, s) && disjoint(tmp1, s, dst, s) && disjoint(src, s, dst, s)) || isInlinableMemmove(dst, src, s, config))
	...
	for {
		...
		tmp2 := midmem.Args[0]
		src := midmem.Args[1]
		if !(t1.(*types.Type).Compare(t2.(*types.Type)) == types.CMPeq && isSamePtr(tmp1, tmp2) && ((disjoint(src, s, tmp2, s) && disjoint(tmp1, s, dst, s) && disjoint(src, s, dst, s)) || isInlinableMemmove(dst, src, s, config))) {
			break
		}
		v.reset(OpMove)
		v.AuxInt = s
