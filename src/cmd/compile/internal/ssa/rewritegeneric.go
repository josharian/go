...
func rewriteValuegeneric_OpMove_20(v *Value) bool {
	...
	// cond: s1 == s2 && midmem.Uses == 1 && t1.(*types.Type).Compare(t2.(*types.Type)) == types.CMPeq && isSamePtr(tmp1, tmp2) && isAutoTmp(tmp1.Aux)
	...
	for {
		...
		src := midmem.Args[1]
		mem := midmem.Args[2]
		if !(s1 == s2 && midmem.Uses == 1 && t1.(*types.Type).Compare(t2.(*types.Type)) == types.CMPeq && isSamePtr(tmp1, tmp2) && isAutoTmp(tmp1.Aux)) {
			break
		}
		v.reset(OpMove)
		v.AuxInt = s1
