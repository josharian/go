...
func (fn *Func) scratchBlocksForDom(maxBlockID int) (a, b, c, d, e, f, g []ID) {
	tot := maxBlockID * nscratchslices
	scratch := fn.domblockstore
	if len(scratch) < tot {
...
func (f *Func) dominatorsLTOrig(entry *Block, predFn linkedBlocks, succFn linkedBlocks) []*Block {
	...
	semi, vertex, label, parent, ancestor, bucketHead, bucketLink := f.scratchBlocksForDom(maxBlockID)
