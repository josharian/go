...
func genRules(arch arch, suffix string) {
	// Open input file.
	filename := arch.name + suffix + ".rules"
	text, err := os.Open(filename)
	if err != nil {
		log.Printf("can't read rule file: %v", err)
		return
	}
	...
	w := new(bytes.Buffer)
	fmt.Fprintf(w, "// autogenerated from gen/%s: do not edit!\n", filename)
	...
	fmt.Fprintf(w, "func rewriteValue%s%s(v *Value, config *Config) bool {\n", arch.name, suffix)
	fmt.Fprintf(w, "switch v.Op {\n")
	for _, op := range ops {
		fmt.Fprintf(w, "case %s:\n", opName(op, arch))
		fmt.Fprintf(w, "return rewriteValue%s%s_%s(v, config)\n", arch.name, suffix, opName(op, arch))
	}
	fmt.Fprintf(w, "}\n")
	...
	for _, op := range ops {
		fmt.Fprintf(w, "func rewriteValue%s%s_%s(v *Value, config *Config) bool {\n", arch.name, suffix, opName(op, arch))
		...
		for _, rule := range oprules[op] {
			...
			if *genLog {
				fmt.Fprintf(w, "fmt.Println(\"rewrite %s:%d\")\n", filename, rule.lineno)
			}
	...
	// Write to file
	err = ioutil.WriteFile("../rewrite"+arch.name+suffix+".go", src, 0666)
	if err != nil {
