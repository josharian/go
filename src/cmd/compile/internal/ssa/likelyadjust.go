...
func describeBranchPrediction(f *Func, b *Block, likely, not int8, prediction BranchPrediction) {
	f.Warnl(b.Line, "Branch prediction rule %s < %s%s",
...
func likelyadjust(f *Func) {
	...
	for _, b := range po {
			...
			if len(b.Succs) == 1 {
				...
				if l != nil && l0 != l1 {
					...
					if f.pass.debug > 0 && !noprediction {
						f.Warnl(b.Line, "Branch prediction rule stay in loop%s",
		...
		if f.pass.debug > 2 {
			f.Warnl(b.Line, "BP: Block %s, local=%s, certain=%s", b, bllikelies[local[b.ID]-blMin], bllikelies[certain[b.ID]-blMin])
		}
