...
func staticassign(l *Node, r *Node, out *[]*Node) bool {
	...
	case OPTRLIT:
		...
		case OARRAYLIT, OSLICELIT, OMAPLIT, OSTRUCTLIT:
			// Init pointer.
			a := staticname(r.Left.Type, false)
...
func staticname(t *Type, readonly bool) *Node {
...
// Static initialization represents literals and literal components of composite literals.
// Dynamic initialization represents non-literals and non-literal components of composite literals.
// LocalCode initialized represents initialization that occurs purely in generated code local to the function of use.
...
func slicelit(ctxt initContext, n *Node, var_ *Node, init *Nodes) {
	...
	mode := getdyn(n, true)
	if mode&initConst != 0 {
		vstat = staticname(t, ctxt == inInitFunction)
		fixedlit(ctxt, initKindStatic, n, vstat, init)
	}
