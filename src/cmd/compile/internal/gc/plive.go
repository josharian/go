...
type Liveness struct {
	...
	progeffectscache progeffectscache
}
...
func (lv *Liveness) initprogeffectscache() {
	if lv.progeffectscache.initialized {
		panic("DOUBLE INIT")
		return
	}
	lv.progeffectscache.initialized = true
	...
	for i, node := range lv.vars {
		...
		case PPARAM:
			...
			// function runs.
			lv.progeffectscache.tailuevar = append(lv.progeffectscache.tailuevar, int32(i))

			if node.Addrtaken {
				lv.progeffectscache.textavarinit = append(lv.progeffectscache.textavarinit, int32(i))
			}
			lv.progeffectscache.textvarkill = append(lv.progeffectscache.textvarkill, int32(i))
		...
		case PPARAMOUT:
			...
			if !node.Addrtaken {
				lv.progeffectscache.retuevar = append(lv.progeffectscache.retuevar, int32(i))
			}
		}
	}
}
...
func (lv *Liveness) progeffects(prog *obj.Prog) (uevar, varkill, avarinit []int32) {
	...
	if (prog.As == obj.AJMP || prog.As == obj.ARET) && prog.To.Type == obj.TYPE_MEM && prog.To.Name == obj.NAME_EXTERN {
		...
		// See issue 16016.
		return lv.progeffectscache.tailuevar, nil, nil
	}

	if prog.As == obj.ARET {
		if prog.To.Type == obj.TYPE_NONE {
			return lv.progeffectscache.retuevar, nil, nil
		}
		return nil, nil, nil
	}
	...
	if prog.As == obj.ATEXT {
		...
		return nil, lv.progeffectscache.textvarkill, lv.progeffectscache.textavarinit
	}

	uevar = lv.progeffectscache.uevar[:0]
	varkill = lv.progeffectscache.varkill[:0]
	avarinit = lv.progeffectscache.avarinit[:0]
...
func livenessprintdebug(lv *Liveness) {
	...
	for i, bb := range lv.cfg {
		...
		for p := bb.first; ; p = p.Link {
			...
			printed = false
			_, _, _ = uevar, varkill, avarinit // TODO
